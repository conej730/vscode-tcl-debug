...
proc dbg::HandleConnect {sock host port} {
	...
	if {$nubSocket != -1} {
		puts "nubSocket does not equal -1 :("
	close $sock
	} else {
	set nubSocket $sock
	set appState running
...
proc dbg::initInstrument {} {
	if {$dbg::appState != "dead"} {
		SendAsync set DbgNub(dynProc)	   $::debugger::instrumentDynamic
		SendAsync set DbgNub(includeFiles) $::debugger::doInstrument
		SendAsync set DbgNub(excludeFiles) $::debugger::dontInstrument
		SendAsync set DbgNub(autoLoad)	 $::debugger::autoLoad
		SendAsync set DbgNub(errorAction)  $::debugger::errorAction
	}
	return
}
